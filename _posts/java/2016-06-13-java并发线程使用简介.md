...
线程既是一个队列的生产者又是这个队列的消费者的时候可以使用到
...
异步执行机制,替换传统的new Thread(runnable).start()方式，使我们能够在后台执行任务；实现了线程池
...
提供比synchronized更灵活的锁方式，允许以三种形式（可中断、不可中断和定时）尝试获取锁，并允许以任何顺序获取和释放多个锁
