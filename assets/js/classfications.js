...
function categoryDisplay() {
   /*only show first cate*/
    ...
    $('.categories-list-item').click(function() {
        ...
        $cur_cate_posts.find("a.post-list-item").slice(0,5).show();//显示文章
         //第一个标签下的页码
...
function generatePagi() {
    ...
    $("div.pagination .inline-list li a").on("click",function(){
        ...
        if(cur_page-1>2){
            ...
            $cur_cate_posts.find("a.post-list-item").slice((cur_page-1)*5,cur_page*5).show(400);//显示文章
            //隐藏所有页码
    ...
    $("div.pagination .prev").on("click",function(){
        ...
        if(prev_page>0){
            ...
            $cur_cate_posts.find("a.post-list-item").hide(250);//隐藏文章
            $cur_cate_posts.find("a.post-list-item").slice((prev_page-1)*5,prev_page*5).show(400);//显示文章
        }else{
           alert("第一页是最小的页码哦!");
        }
    });
    ...
    $("div.pagination .next").on("click",function(){
        ...
        if(next_page<=total_page){
            ...
            $cur_cate_posts.find("a.post-list-item").hide(250);//隐藏文章
            $cur_cate_posts.find("a.post-list-item").slice((next_page-1)*5,next_page*5).show(400);//显示文章
        }else{
